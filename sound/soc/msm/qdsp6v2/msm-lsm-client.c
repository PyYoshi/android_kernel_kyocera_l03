...
static struct wake_lock listen_wake_lock;
	...
	case SNDRV_LSM_EVENT_STATUS:
		...
		if (!rc && !xchg) {
			pr_debug("%s: New event available %ld\n", __func__,
				 prtd->event_avail);
			wake_lock_timeout(&listen_wake_lock, HZ);
			spin_lock_irqsave(&prtd->event_lock, flags);
...
static __devinit int msm_lsm_probe(struct platform_device *pdev)
{
	...
	wake_lock_init(&listen_wake_lock, WAKE_LOCK_SUSPEND, "listen_kernel");
...
static int msm_lsm_remove(struct platform_device *pdev)
{
	...
	wake_lock_destroy(&listen_wake_lock);
