...
static __devinit int msm8974_asoc_machine_probe(struct platform_device *pdev)
{
	...
	if (jack_gpio < 0) {
		...
		dev_err(&pdev->dev,
			"Looking up %s property in node %s failed %d\n",
			"qcom, jack-gpios", pdev->dev.of_node->full_name,
			jack_gpio);
		ret = -ENODEV;
		goto err;
	}
	...
	if (mbhc_cfg.calibration) {
		ret = taiko_hs_detect(g_rtd->codec, &mbhc_cfg);
	    if (ret){
			goto err;
	    }
	} else {
		ret = -ENOMEM;
	    pr_err("mbhc_cfg.calibration none\n");
		goto err;
	}
