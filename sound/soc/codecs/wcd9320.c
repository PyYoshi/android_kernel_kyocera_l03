		...
		if (!strncmp(card->dapm_routes[i].sink,
			...
			if (strnstr(card->dapm_routes[i].source,
				...
				if(OEM_BOARD3_TYPE == OEM_get_board()){
					snd_soc_update_bits(codec, TAIKO_A_MICB_3_INT_RBIAS, 0xE0, 0x80);//0x138
					snd_soc_update_bits(codec, TAIKO_A_MICB_3_CTL, 0xF0, 0xD0);//0x137
				}
				break;
			} else if (strnstr(card->dapm_routes[i].source,
				  ...
				  struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_POST_PMD:
		if (OEM_BOARD3_TYPE == OEM_get_board()){
			snd_soc_update_bits(codec, TAIKO_A_MICB_3_INT_RBIAS, 0x80, 0x00);//0x138
			snd_soc_update_bits(codec, TAIKO_A_MICB_3_CTL, 0x80, 0x00);//0x137
		}
		break;
	}
	return ret;
	...
	struct snd_kcontrol *kcontrol, int event)
{
	...
	case SND_SOC_DAPM_PRE_PMU:
		...
		else if ((OEM_BOARD3_TYPE == OEM_get_board()) && (strnstr(w->name, "MIC BIAS3", sizeof("MIC BIAS3"))))
	        snd_soc_update_bits(codec, micb_int_reg, 0xE0, 0x80);
...
static const struct snd_soc_dapm_widget taiko_dapm_widgets[] = {
	...
	SND_SOC_DAPM_AIF_OUT_E("AIF4 MAD", "AIF4 MAD TX", 0,
			       SND_SOC_NOPM, 0, 0,
			       taiko_codec_enable_mad, SND_SOC_DAPM_PRE_PMU | SND_SOC_DAPM_POST_PMD),
