...
static const struct qpnp_vadc_map_pt adcmap_nonpull_oem_WS[] = {
	{1792,	-40},
	{1788,	-35},
	{1783,	-30},
	{1777,	-25},
		...
		struct qpnp_vadc_result *adc_chan_result)
{
	...
	if (OEM_get_board() == OEM_BOARD1_TYPE) {
		qpnp_adc_map_voltage_temp(adcmap_nonpull_oem,
			ARRAY_SIZE(adcmap_nonpull_oem),
			therm_voltage, &adc_chan_result->physical);
	}else {
		qpnp_adc_map_voltage_temp(adcmap_nonpull_oem_WS,
			ARRAY_SIZE(adcmap_nonpull_oem_WS),
			therm_voltage, &adc_chan_result->physical);
	}
