...
static int32_t g_thp7212_res=0;
static int32_t g_thp7212_prev_w, g_thp7212_prev_h;
static int32_t g_thp7212_bracket_mode = 0;
static int32_t g_thp7212_module_bracket = 0;
static int32_t g_thp7212_af_roi = 0;
static int32_t g_thp7212_af_mode = 0;
static int32_t g_thp7212_snapshot_flg = 0;
static int32_t g_thp7212_multi_frame_save = 0;
static int32_t g_thp7212_multi_frame_save_parm = 0;
static int32_t g_thp7212_led_mode = 0;
static int32_t g_thp7212_stream_mask = 0;
static int32_t g_thp7212_edata = 0;
static int32_t g_thp7212_fd_ex = 0;
static int32_t g_thp7212_stream_flg = 0;
...
static struct thp7212_mapping thp7212_map_bestshot[] = {
    ...
    {CAMERA_SCENE_MODE_PET        , 0xD},
    {CAMERA_SCENE_MODE_LANDSCAPE_P, 0xE},
    {CAMERA_SCENE_MODE_NIGHT_ASD  , 0xF},
    {CAMERA_SCENE_MODE_AUTO       , 0xFF},
};

/* Focus Control */
static struct msm_camera_i2c_reg_conf thp7212_focus_single_face[] = {
    {0xF032, 0x00},
    {0xF041, 0x02},
    {0xF040, 0x01},
};
...
static struct msm_camera_i2c_reg_conf thp7212_focus_start[] = {
    {0xF040, 0x01},
};

static struct msm_camera_i2c_reg_conf thp7212_focus_stop[] = {
    {0xF040, 0x80},
};
...
static struct msm_camera_i2c_reg_conf thp7212_focus_caf_video_face[] = {
    {0xF041, 0x12},
    {0xF040, 0x01},
};
...
static struct msm_camera_i2c_reg_conf thp7212_focus_caf_pict_face[] = {
    {0xF041, 0x32},
    {0xF040, 0x01},
};
...
static struct msm_camera_i2c_reg_conf thp7212_focus_multi[] = {
    {0xF032, 0x00},
    {0xF041, 0x04},
    {0xF040, 0x01},
};

static struct msm_camera_i2c_reg_conf thp7212_focus_multi_1_1[] = {
    {0xF032, 0x00},
    {0xF041, 0x05},
    {0xF040, 0x01},
};
...
static struct msm_camera_i2c_reg_conf thp7212_af_roi_parm_pcaf[] = {
    {0xF046, 0x00},
    {0xF047, 0x00},
    {0xF048, 0x00},
    {0xF049, 0x00},
...
static struct thp7212_conf_array_setting_size thp7212_confs[] = {
    ...
    {&thp7212_full_size_wvga,         1,  0x07},
    {&thp7212_full_size_3m_hdr,       2,  0x02},
    {&thp7212_full_size_fhd_plus_hdr, 2,  0x02},
};
...
static int32_t thp7212_set_resolution(struct msm_sensor_ctrl_t *s_ctrl, int32_t res, uint32_t stream_mask, int32_t prev_w, int32_t prev_h)
{
    ...
    g_thp7212_prev_w = prev_w;
    g_thp7212_prev_h = prev_h;
    if((g_thp7212_res == res) && (g_thp7212_stream_mask == stream_mask))
    {
      if(g_thp7212_module_bracket == g_thp7212_bracket_mode)
      {
        g_thp7212_stream_mask = stream_mask;
        if(g_thp7212_bracket_mode == CAMERA_BRACKET_MODE_ON) {
          if((stream_mask == 0x0C) && (g_thp7212_module_bracket == 1)) {
            return 0;
          }
        } else
          return 0;
    ...
    if((g_thp7212_bracket_mode == CAMERA_BRACKET_MODE_ON) && (stream_mask == 0x0C) && (g_thp7212_module_bracket == 0)) {
        ...
        g_thp7212_module_bracket = 1;
        e_data = 0x05;
    } else if(stream_mask == 0x0C) {
        thp7212_full_size_13m[3].reg_data = 0x03;
        thp7212_full_size_9m[3].reg_data = 0x03;
        thp7212_full_size_8m[3].reg_data = 0x03;
        thp7212_full_size_3m[3].reg_data = 0x03;
        thp7212_full_size_fhd[3].reg_data = 0x03;
        thp7212_full_size_fhd_plus[3].reg_data = 0x03;
        g_thp7212_module_bracket = 0;
        e_data = 0x03;
    } else {
    ...
    pr_info("CFG_SET_RESOLUTION led_mode:%d\n", g_thp7212_led_mode);
    //led ctrl
    thp7212_full_size_13m[2].reg_data = 0x00;
    thp7212_full_size_9m[2].reg_data = 0x00;
    thp7212_full_size_8m[2].reg_data = 0x00;
    thp7212_full_size_3m[2].reg_data = 0x00;
    thp7212_full_size_fhd[2].reg_data = 0x00;
    thp7212_full_size_fhd_plus[2].reg_data = 0x00;
    if(g_thp7212_led_mode == 2){//on
      thp7212_full_size_13m[2].reg_data = 0x02;
      thp7212_full_size_9m[2].reg_data = 0x02;
      thp7212_full_size_8m[2].reg_data = 0x02;
      thp7212_full_size_3m[2].reg_data = 0x02;
      thp7212_full_size_fhd[2].reg_data = 0x02;
      thp7212_full_size_fhd_plus[2].reg_data = 0x02;
    }else if(g_thp7212_led_mode == 1){//auto
      thp7212_full_size_13m[2].reg_data = 0x01;
      thp7212_full_size_9m[2].reg_data = 0x01;
      thp7212_full_size_8m[2].reg_data = 0x01;
      thp7212_full_size_3m[2].reg_data = 0x01;
      thp7212_full_size_fhd[2].reg_data = 0x01;
      thp7212_full_size_fhd_plus[2].reg_data = 0x01;
    }

    if(e_data == 0x03){
        rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(
            s_ctrl->sensor_i2c_client,
            0xF012, 0x0002, MSM_CAMERA_I2C_BYTE_DATA);
    }
    ...
    if(stream_mask != 0x0C) {
      ...
      g_thp7212_snapshot_flg = 1;
    if(g_thp7212_multi_frame_save)
    {
        pr_err("multi_frame set val:%d",g_thp7212_multi_frame_save_parm);
        rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(
            s_ctrl->sensor_i2c_client,
            0xF01A, (uint16_t)g_thp7212_multi_frame_save_parm, MSM_CAMERA_I2C_BYTE_DATA);
        if (rc) {
            pr_err("%s:%d failed\n", __func__, __LINE__);
            msm_error_notify((unsigned int)s_ctrl->sensordata->sensor_info->session_id, MSM_CAMERA_PRIV_I2C_ERROR);
        }
        g_thp7212_multi_frame_save = 0;
    }

    g_thp7212_res = res;
    g_thp7212_edata = e_data;
    return 0;
}
...
static int32_t thp7212_start_stream(struct msm_sensor_ctrl_t *s_ctrl)
{
    ...
    if(g_thp7212_snapshot_flg == 1){
      e_data = g_thp7212_edata;
#if 0
      e_data = thp7212_confs[g_thp7212_res].e_data;
      if((g_thp7212_module_bracket == 1) &&
          (g_thp7212_res <= MSM_SENSOR_RES_5)) {
          e_data = 0x05;
      }
#endif
      for (i=0; i<300; i++) {
...
static int32_t thp7212_set_focus_mode(struct msm_sensor_ctrl_t *s_ctrl, struct sensor_af_info_t *af_info)
{
    ...
    if(af_info->mode == g_thp7212_af_mode)
        return 0;
    ...
    if( af_info->mode == FOCUS_MODE_OFF ){
        rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write_conf_tbl(
            s_ctrl->sensor_i2c_client,
            thp7212_focus_stop,
            ARRAY_SIZE(thp7212_focus_stop),
            MSM_CAMERA_I2C_BYTE_DATA);
    }
#endif
    if( af_info->mode == FOCUS_MODE_MULTI ){
#if 0
        if( af_info->aspect_ratio == 0 ){ //1:1
            rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write_conf_tbl(
              s_ctrl->sensor_i2c_client,
              thp7212_focus_multi_1_1,
              ARRAY_SIZE(thp7212_focus_multi_1_1),
              MSM_CAMERA_I2C_BYTE_DATA);
        }
        else{
            rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write_conf_tbl(
              s_ctrl->sensor_i2c_client,
              thp7212_focus_multi,
              ARRAY_SIZE(thp7212_focus_multi),
              MSM_CAMERA_I2C_BYTE_DATA);
        }
#endif
    }
        ...
        if(g_thp7212_af_roi == 1){
            thp7212_af_roi_parm[8].reg_data = 0x00;
            thp7212_af_roi_parm[9].reg_data = 0x02;
            thp7212_af_roi_parm[10].reg_data = 0x02;
            if(g_thp7212_led_mode == 2){//on
              thp7212_af_roi_parm[8].reg_data = 0x02;
              thp7212_af_roi_parm[9].reg_data = 0x03;
              thp7212_af_roi_parm[10].reg_data = 0x01;
            } else if(g_thp7212_led_mode == 1){//auto
              thp7212_af_roi_parm[8].reg_data = 0x01;
              thp7212_af_roi_parm[9].reg_data = 0x03;
              thp7212_af_roi_parm[10].reg_data = 0x01;
            }
            rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write_conf_tbl(
      ...
      if( g_thp7212_fd_ex == 0){
        ...
        rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write_conf_tbl(
            s_ctrl->sensor_i2c_client,
            thp7212_focus_caf_video_face,
            ARRAY_SIZE(thp7212_focus_caf_video_face),
            MSM_CAMERA_I2C_BYTE_DATA);
      }
    }
      ...
      if( g_thp7212_fd_ex == 0){
        ...
        rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write_conf_tbl(
            s_ctrl->sensor_i2c_client,
            thp7212_focus_caf_pict_face,
            ARRAY_SIZE(thp7212_focus_caf_pict_face),
            MSM_CAMERA_I2C_BYTE_DATA);
      }
    }
    if (rc) {
...
static int32_t thp7212_set_af_roi(struct msm_sensor_ctrl_t *s_ctrl, struct sensor_af_roi_info_t *af_roi)
{
    ...
    g_thp7212_af_roi = 0;
    if (af_roi->width == thp7212_sensor_output_size[g_thp7212_res].width && 
        af_roi->height == thp7212_sensor_output_size[g_thp7212_res].height) {
      ...
      } else if (g_thp7212_af_mode == FOCUS_MODE_CONTINOUS_PICTURE) {
        if (g_thp7212_fd_ex == 0) {
          rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write_conf_tbl(
              s_ctrl->sensor_i2c_client,
              thp7212_focus_caf_pict,
...
#if 0
        if (g_thp7212_fd_ex == 0) {
          thp7212_focus_single[0].reg_data = 0x00;
          rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write_conf_tbl(
              s_ctrl->sensor_i2c_client,
              thp7212_focus_single,
              ARRAY_SIZE(thp7212_focus_single),
          ...
          thp7212_focus_single_face[0].reg_data = 0x00;
          rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write_conf_tbl(
              s_ctrl->sensor_i2c_client,
              thp7212_focus_single_face,
              ARRAY_SIZE(thp7212_focus_single_face),
              MSM_CAMERA_I2C_BYTE_DATA);
        }
#endif
      }
    } else {
      if (g_thp7212_af_mode == FOCUS_MODE_CONTINOUS_VIDEO && g_thp7212_fd_ex == 0) {
      ...
      } else if (g_thp7212_af_mode == FOCUS_MODE_CONTINOUS_PICTURE && g_thp7212_fd_ex == 0) {
        thp7212_af_roi_parm_pcaf[0].reg_data = (left >> 8) & 0xff;
        thp7212_af_roi_parm_pcaf[1].reg_data = left & 0xff;
        thp7212_af_roi_parm_pcaf[2].reg_data = (top >> 8) & 0xff;
        thp7212_af_roi_parm_pcaf[3].reg_data = top & 0xff;
        thp7212_af_roi_parm_pcaf[4].reg_data = (right >> 8) & 0xff;
        thp7212_af_roi_parm_pcaf[5].reg_data = right & 0xff;
        thp7212_af_roi_parm_pcaf[6].reg_data = (bottom >> 8) & 0xff;
        thp7212_af_roi_parm_pcaf[7].reg_data = bottom & 0xff;

        rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write_conf_tbl(
            s_ctrl->sensor_i2c_client,
            thp7212_af_roi_parm_pcaf,
            ARRAY_SIZE(thp7212_af_roi_parm_pcaf),
            MSM_CAMERA_I2C_BYTE_DATA);
        if (rc) {
            pr_err("%s:%d failed\n", __func__, __LINE__);
            msm_error_notify((unsigned int)s_ctrl->sensordata->sensor_info->session_id, MSM_CAMERA_PRIV_I2C_ERROR);
...
static int32_t thp7212_set_focus_ctrl(struct msm_sensor_ctrl_t *s_ctrl, struct sensor_af_ctrl_info_t  *af_ctrl_info)
{
    int32_t rc=0;

    pr_info("%s af_ctrl:%d mode:%d",__func__, af_ctrl_info->af_ctrl, g_thp7212_af_mode);

    if (af_ctrl_info->af_ctrl == FOCUS_CTRL_DO_AF) {
      if (g_thp7212_af_mode == FOCUS_MODE_AUTO && g_thp7212_af_roi == 1){
        if (g_thp7212_fd_ex == 0) {
            thp7212_af_roi_parm[8].reg_data = 0x00;
            thp7212_af_roi_parm[9].reg_data = 0x02;
            thp7212_af_roi_parm[10].reg_data = 0x02;
            ...
            thp7212_focus_single_face[0].reg_data = 0x00;
            if(g_thp7212_led_mode == 2)//on
              thp7212_focus_single_face[0].reg_data = 0x02;
            else if(g_thp7212_led_mode == 1)//auto
              thp7212_focus_single_face[0].reg_data = 0x01;
      ...
      } else if (g_thp7212_af_mode == FOCUS_MODE_MULTI){
        if( af_ctrl_info->aspect_ratio == 0 ){ //1:1
            thp7212_focus_multi_1_1[0].reg_data = 0x00;
            if(g_thp7212_led_mode == 2)//on
              thp7212_focus_multi_1_1[0].reg_data = 0x02;
            else if(g_thp7212_led_mode == 1)//auto
              thp7212_focus_multi_1_1[0].reg_data = 0x01;
            rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write_conf_tbl(
              s_ctrl->sensor_i2c_client,
              thp7212_focus_multi_1_1,
              ARRAY_SIZE(thp7212_focus_multi_1_1),
            ...
            thp7212_focus_multi[0].reg_data = 0x00;
            if(g_thp7212_led_mode == 2)//on
              thp7212_focus_multi[0].reg_data = 0x02;
            else if(g_thp7212_led_mode == 1)//auto
              thp7212_focus_multi[0].reg_data = 0x01;
            rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write_conf_tbl(
              s_ctrl->sensor_i2c_client,
              thp7212_focus_multi,
              ARRAY_SIZE(thp7212_focus_multi),
      ...
      } else if (g_thp7212_af_mode == FOCUS_MODE_MACRO){
          //nothing
      } else if (g_thp7212_af_mode == FOCUS_MODE_CONTINOUS_VIDEO || g_thp7212_af_mode == FOCUS_MODE_CONTINOUS_PICTURE) {
          rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write_conf_tbl(
              s_ctrl->sensor_i2c_client,
              thp7212_focus_stop,
              ARRAY_SIZE(thp7212_focus_stop),
        ...
        if (g_thp7212_fd_ex == 0) {
          thp7212_focus_single[0].reg_data = 0x00;
          if(g_thp7212_led_mode == 2){//on
            thp7212_focus_single[0].reg_data = 0x02;
          } else if(g_thp7212_led_mode == 1){//auto
            thp7212_focus_single[0].reg_data = 0x01;
          }
          rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write_conf_tbl(
              s_ctrl->sensor_i2c_client,
              thp7212_focus_single,
      ...
      if (g_thp7212_af_mode == FOCUS_MODE_CONTINOUS_VIDEO || g_thp7212_af_mode == FOCUS_MODE_CONTINOUS_PICTURE) {
          rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write_conf_tbl(
              s_ctrl->sensor_i2c_client,
              thp7212_focus_start,
              ARRAY_SIZE(thp7212_focus_start),
              MSM_CAMERA_I2C_BYTE_DATA);
      } else {
          rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write_conf_tbl(
              s_ctrl->sensor_i2c_client,
...
static int32_t thp7212_set_fd_ex(struct msm_sensor_ctrl_t *s_ctrl, int32_t fd_ex)
{
    int32_t rc=0;
    pr_info("%s val:%d", __func__, fd_ex);
    ...
    rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(
        s_ctrl->sensor_i2c_client,
        0xF064, (uint16_t)fd_ex, MSM_CAMERA_I2C_BYTE_DATA);
    if (rc) {
        pr_err("%s:%d failed\n", __func__, __LINE__);
        msm_error_notify((unsigned int)s_ctrl->sensordata->sensor_info->session_id, MSM_CAMERA_PRIV_I2C_ERROR);
    }
...
static int32_t thp7212_set_multi_frame(struct msm_sensor_ctrl_t *s_ctrl, int32_t multi_frame)
{
    ...
    if(g_thp7212_res==6)
    {
        g_thp7212_multi_frame_save = 1;
        g_thp7212_multi_frame_save_parm = multi_frame;
...
static int32_t thp7212_set_zoom(struct msm_sensor_ctrl_t *s_ctrl, int32_t zoom)
{
    ...
    pr_info("%s a_val:%d", __func__, zoom);
...
static int32_t thp7212_set_asd_lock(struct msm_sensor_ctrl_t *s_ctrl, int32_t lock)
{
    int32_t rc=0;
    pr_info("%s val:%d", __func__, lock);
    ...
    rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(
        s_ctrl->sensor_i2c_client,
        0xF05F, lock, MSM_CAMERA_I2C_BYTE_DATA);
    if (rc) {
        pr_err("%s:%d failed\n", __func__, __LINE__);
        msm_error_notify((unsigned int)s_ctrl->sensordata->sensor_info->session_id, MSM_CAMERA_PRIV_I2C_ERROR);
    }
...
static int32_t thp7212_set_caf_lock(struct msm_sensor_ctrl_t *s_ctrl, int32_t lock)
{
    int32_t rc=0;
    pr_info("%s val:%d", __func__, lock);
...
static int32_t thp7212_set_led(struct msm_sensor_ctrl_t *s_ctrl, int32_t led)
{
    int32_t rc=0;
    pr_info("%s val:%d", __func__, led);

    //current state check

    rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(
        s_ctrl->sensor_i2c_client,
        0xF031, led, MSM_CAMERA_I2C_BYTE_DATA);
    if (rc) {
        pr_err("%s:%d failed\n", __func__, __LINE__);
        msm_error_notify((unsigned int)s_ctrl->sensordata->sensor_info->session_id, MSM_CAMERA_PRIV_I2C_ERROR);
    }
...
static int32_t thp7212_set_led_mode(struct msm_sensor_ctrl_t *s_ctrl, int32_t led_mode)
{
    int32_t rc=0;
    pr_info("%s val:%d", __func__, led_mode);

    g_thp7212_led_mode = led_mode;

    return rc;
}

static int32_t thp7212_set_capt_30fps(struct msm_sensor_ctrl_t *s_ctrl, int32_t fps_ctrl)
{
    int32_t rc=0;
    pr_info("%s val:%d", __func__, fps_ctrl);

    //current state check

    rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(
        s_ctrl->sensor_i2c_client,
        0xF00F, fps_ctrl, MSM_CAMERA_I2C_BYTE_DATA);
    if (rc) {
        pr_err("%s:%d failed\n", __func__, __LINE__);
        msm_error_notify((unsigned int)s_ctrl->sensordata->sensor_info->session_id, MSM_CAMERA_PRIV_I2C_ERROR);
    }
...
static int32_t thp7212_get_face_detect_ex(struct msm_sensor_ctrl_t *s_ctrl, void *setting)
{
    ...
    if (g_thp7212_prev_w == g_thp7212_prev_h) {
        coef_y = 1000 * thp7212_sensor_output_size[g_thp7212_res].height / g_thp7212_prev_h;
        coef_x = coef_y;
    } else {
        coef_x = 1000 * thp7212_sensor_output_size[g_thp7212_res].width  / g_thp7212_prev_w;
        coef_y = 1000 * thp7212_sensor_output_size[g_thp7212_res].height / g_thp7212_prev_h;
    }
    correction = (thp7212_sensor_output_size[g_thp7212_res].width - thp7212_sensor_output_size[g_thp7212_res].height) / 2;
    /* Face pos read */
    for(i=0,j=0; i<fd_num; i++)
    {
        ...
        if (g_thp7212_prev_w == g_thp7212_prev_h) {
            if( ((int16_t)(fd_data[0] << 8 | fd_data[1]) >= correction) &&
                ...
                ((fd_data[4] << 8 | fd_data[5]) <= (thp7212_sensor_output_size[g_thp7212_res].width-correction)) &&
                ((fd_data[6] << 8 | fd_data[7]) <= thp7212_sensor_output_size[g_thp7212_res].height) ) {
            ...
            if( ((int16_t)(fd_data[0] << 8 | fd_data[1]) >= 0) &&
                ((int16_t)(fd_data[2] << 8 | fd_data[3]) >= 0) &&
                ((fd_data[4] << 8 | fd_data[5]) <= thp7212_sensor_output_size[g_thp7212_res].width) &&
                ((fd_data[6] << 8 | fd_data[7]) <= thp7212_sensor_output_size[g_thp7212_res].height) ) {
...
static int32_t thp7212_get_illumi_estimate(struct msm_sensor_ctrl_t *s_ctrl, void *setting)
{
    ...
    rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(
		s_ctrl->sensor_i2c_client, 0xF02C, &data, MSM_CAMERA_I2C_BYTE_DATA);
    if (rc < 0) {
        pr_err("%s:%d: i2c_read failed\n", __func__, __LINE__);
        msm_error_notify((unsigned int)s_ctrl->sensordata->sensor_info->session_id, MSM_CAMERA_PRIV_I2C_ERROR);
        return rc;
...
static int32_t thp7212_get_af_status_multi(struct msm_sensor_ctrl_t *s_ctrl, void *setting)
{
    int32_t rc=0;
    uint8_t data[2];
    struct sensor_focus_multi_info_t multi_info;

    rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read_seq(
        s_ctrl->sensor_i2c_client, 0xF04E, data, sizeof(data));
    if (rc < 0) {
        pr_err("%s:%d: i2c_read failed\n", __func__, __LINE__);
        msm_error_notify((unsigned int)s_ctrl->sensordata->sensor_info->session_id, MSM_CAMERA_PRIV_I2C_ERROR);
        return rc;
    ...
    pr_info("%s [%02x][%02x]\n", __func__, data[0], data[1] );
    multi_info.focus_area = (uint16_t)(data[0]<<1) | (data[1]>>4 & 0x01);
    multi_info.best_focus_area = (data[1] & 0x0F);
    pr_info("%s [%08x][%08x]\n", __func__, multi_info.focus_area, multi_info.best_focus_area );

    if (copy_to_user(setting, (void *)&multi_info, sizeof(multi_info))) {
        pr_err("%s:%d copy failed\n", __func__, __LINE__);
        rc = -EFAULT;
    }
    return rc;
...
static int32_t thp7212_get_iso_speed(struct msm_sensor_ctrl_t *s_ctrl, void *setting)
{
    int32_t rc=0;
    uint8_t data[2];
    uint16_t iso_speed;
    ...
    rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read_seq(
        s_ctrl->sensor_i2c_client, 0xF02A, data, sizeof(data));
    if (rc < 0) {
        pr_err("%s:%d: i2c_read failed\n", __func__, __LINE__);
        msm_error_notify((unsigned int)s_ctrl->sensordata->sensor_info->session_id, MSM_CAMERA_PRIV_I2C_ERROR);
        return rc;
    }
    iso_speed = ((data[0]<<8) | data[1]);
    pr_info("%s [%d]\n", __func__, iso_speed );

    if (copy_to_user(setting, (void *)&iso_speed, sizeof(uint16_t))) {
        pr_err("%s:%d copy failed\n", __func__, __LINE__);
        rc = -EFAULT;
    }
...
static int32_t thp7212_get_metering_mode(struct msm_sensor_ctrl_t *s_ctrl, void *setting)
{
    int32_t rc=0;
    uint16_t data;
    uint16_t ae_metering;

    rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(
		s_ctrl->sensor_i2c_client, 0xF02D, &data, MSM_CAMERA_I2C_BYTE_DATA);
    if (rc < 0) {
        pr_err("%s:%d: i2c_read failed\n", __func__, __LINE__);
        msm_error_notify((unsigned int)s_ctrl->sensordata->sensor_info->session_id, MSM_CAMERA_PRIV_I2C_ERROR);
        return rc;
    ...
    if (copy_to_user(setting, (void *)&ae_metering, sizeof(uint16_t))) {
        pr_err("%s:%d copy failed\n", __func__, __LINE__);
        rc = -EFAULT;
    }
    return rc;
...
static int32_t thp7212_get_flicker_status(struct msm_sensor_ctrl_t *s_ctrl, void *setting)
{
    int32_t rc=0;
    uint16_t data;

    rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(
		s_ctrl->sensor_i2c_client, 0xF024, &data, MSM_CAMERA_I2C_BYTE_DATA);
    if (rc < 0) {
        pr_err("%s:%d: i2c_read failed\n", __func__, __LINE__);
        msm_error_notify((unsigned int)s_ctrl->sensordata->sensor_info->session_id, MSM_CAMERA_PRIV_I2C_ERROR);
        return rc;
...
static int32_t thp7212_get_led_status(struct msm_sensor_ctrl_t *s_ctrl, void *setting)
{
    int32_t rc=0;
    uint16_t data;

    rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(
		s_ctrl->sensor_i2c_client, 0xF033, &data, MSM_CAMERA_I2C_BYTE_DATA);
    if (rc < 0) {
        pr_err("%s:%d: i2c_read failed\n", __func__, __LINE__);
        msm_error_notify((unsigned int)s_ctrl->sensordata->sensor_info->session_id, MSM_CAMERA_PRIV_I2C_ERROR);
        return rc;
...
static int32_t thp7212_set_sharpness(struct msm_sensor_ctrl_t *s_ctrl, int32_t sharpness)
{
    int32_t rc=0;
    pr_info("%s val:%d", __func__, sharpness);
    ...
    rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(
        s_ctrl->sensor_i2c_client,
        0xF053, sharpness, MSM_CAMERA_I2C_BYTE_DATA);
    if (rc) {
        pr_err("%s:%d failed\n", __func__, __LINE__);
        msm_error_notify((unsigned int)s_ctrl->sensordata->sensor_info->session_id, MSM_CAMERA_PRIV_I2C_ERROR);
    }
...
static int32_t thp7212_lens_move_inf(struct msm_sensor_ctrl_t *s_ctrl)
{
    int32_t rc;
    int32_t i;
    uint16_t data=0;
    ...
    rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_write(
        s_ctrl->sensor_i2c_client,
        0xF040, 0x10, MSM_CAMERA_I2C_BYTE_DATA);
    if (rc) {
        pr_err("%s:%d failed rc:%d\n", __func__, __LINE__, rc);
        return rc;
    }
    ...
    for (i=0; i<50; i++){
        rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(
                s_ctrl->sensor_i2c_client,
                0xF042, &data, MSM_CAMERA_I2C_BYTE_DATA);
        if(~data & 0x80)
            break;
        usleep_range(10000, 10000);
    }
    if(i>=50){
        pr_err("%s:%d timeout err\n", __func__, __LINE__);
        return -EIO;
    }
    ...
    for (i=0; i<50; i++){
        rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(
                s_ctrl->sensor_i2c_client,
                0xF043, &data, MSM_CAMERA_I2C_BYTE_DATA);
        if(data == 0x00)
            break;
        usleep_range(10000, 10000);
    }
    ...
    void __user *argp)
{
    ...
    case CFG_POWER_UP:
        ...
        g_thp7212_af_mode = 0;
        g_thp7212_snapshot_flg = 0;
        g_thp7212_led_mode = 0;
        g_thp7212_fd_ex = 0;
        g_thp7212_stream_flg = 0;
        break;

    case CFG_POWER_DOWN:
        thp7212_lens_move_inf(s_ctrl);
    ...
    case CFG_SET_AWB_LOCK:
        ...
        break;
    case CFG_SET_LED:
        rc = thp7212_set_led(s_ctrl, cdata->cfg.led);
        break;
    case CFG_SET_LED_MODE:
        rc = thp7212_set_led_mode(s_ctrl, cdata->cfg.led_mode);
        break;
    ...
    case CFG_GET_AF_STATUS:
        ...
        break;
    case CFG_GET_AF_STATUS_MULTI:
        rc = thp7212_get_af_status_multi(s_ctrl, cdata->cfg.setting);
        break;
    case CFG_GET_FW_VER:
    ...
    case CFG_SET_ASD_LOCK:
        rc = thp7212_set_asd_lock(s_ctrl, cdata->cfg.lock);
        break;
    case CFG_GET_ISO_SPEED:
        rc = thp7212_get_iso_speed(s_ctrl, cdata->cfg.setting);
        break;
    case CFG_GET_METERING_MODE:
        rc = thp7212_get_metering_mode(s_ctrl, cdata->cfg.setting);
        break;
    case CFG_SET_FOCUS_CTRL:{
        struct sensor_af_ctrl_info_t *af_ctrl_info;
        af_ctrl_info = (struct sensor_af_ctrl_info_t *)&cdata->cfg.af_ctrl_info;
        rc = thp7212_set_focus_ctrl(s_ctrl, af_ctrl_info);
        break;
    }
    case CFG_SET_FD_EX:
        rc = thp7212_set_fd_ex(s_ctrl, cdata->cfg.fd_ex);
        break;
    case CFG_SET_CAPT_30FPS:
        rc = thp7212_set_capt_30fps(s_ctrl, cdata->cfg.fps_ctrl);
        break;
    case CFG_GET_FLICKER_STATUS:
        rc = thp7212_get_flicker_status(s_ctrl, cdata->cfg.setting);
        break;
    case CFG_SET_CAF_LOCK:
        rc = thp7212_set_caf_lock(s_ctrl, cdata->cfg.lock);
        break;
    case CFG_GET_LED_STATUS:
        rc = thp7212_get_led_status(s_ctrl, cdata->cfg.setting);
        break;
    case CFG_SET_SHARPNESS:{
        int32_t sharpness;
        if (copy_from_user(&sharpness, (void *)cdata->cfg.setting,
            sizeof(int32_t))) {
            pr_err("%s:%d failed\n", __func__, __LINE__);
            rc = -EFAULT;
            break;
        }
        rc = thp7212_set_sharpness(s_ctrl, sharpness);
        break;
    }
    default:
        break;
