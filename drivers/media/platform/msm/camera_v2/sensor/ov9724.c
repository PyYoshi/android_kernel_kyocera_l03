...
static int32_t ov9724_get_iso_speed(struct msm_sensor_ctrl_t *s_ctrl, void *setting)
{
    int32_t rc=0;
    uint16_t data;
    uint16_t gain;
    ...
    rc = s_ctrl->sensor_i2c_client->i2c_func_tbl->i2c_read(
            ...
            0x0205, &data, MSM_CAMERA_I2C_BYTE_DATA);
    pr_info("%s: data:0x%04x", __func__, data);

    gain = (1+((data>>6) & 0x01))*(1+((data>>5) & 0x01))*(1+((data>>4) & 0x01))*100;
    gain = (gain * (100+((data & 0x0F)*100)/16))/227;

    pr_info("%s: gain:0x%04x", __func__, gain);

    if (copy_to_user(setting, (void *)&gain, sizeof(uint16_t))) {
...
static struct msm_sensor_fn_t ov9724_sensor_func_tbl = {
    .sensor_config = msm_sensor_config,
    .sensor_power_up = msm_sensor_power_up,
    .sensor_power_down = msm_sensor_power_down,
    .sensor_match_id = msm_sensor_match_id,
    .sensor_get_iso_speed = ov9724_get_iso_speed,
};
...
static struct msm_sensor_ctrl_t ov9724_s_ctrl = {
    ...
    .func_tbl = &ov9724_sensor_func_tbl,
};
