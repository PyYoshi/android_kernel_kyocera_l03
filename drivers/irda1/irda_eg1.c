...
static int irdaeg_suspend (struct platform_device *pdev, pm_message_t state)
{
/*	printk( "%s eg1 en:%d\n", __func__, __LINE__ ); */
...
static int irdaeg_resume (struct platform_device *pdev)
{
/*	printk( "%s eg1 en:%d\n", __func__, __LINE__ ); */
...
//	.remove = __devexit_p(irdaeg_remove),
	...
	.suspend	= irdaeg_suspend,
	.resume		= irdaeg_resume,
};
