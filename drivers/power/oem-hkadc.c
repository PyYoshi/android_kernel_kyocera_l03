...
#ifdef OEM_ENABLE_USB_THERM_OVERHEAT
...
struct power_supply *hkadc_oem_batt_psy = NULL;
...
static int oem_hkadc_init_battery_temp(void)
{
	...
	rc = oem_chg_vadc_read(LR_MUX1_BATT_THERM, &result);
	if (rc) {
		CHG_HKADC_ERR("error reading adc channel = %d, rc = %d\n",
...
static int oem_hkadc_init_pa_therm(void)
{
	...
	rc = oem_chg_vadc_read(LR_MUX6_PU2_AMUX_THM3, &result);
	if (rc) {
...
static int oem_hkadc_init_camera_therm(void)
{
	...
	rc = oem_chg_vadc_read(LR_MUX4_AMUX_THM1, &result);
	if (rc) {
		CHG_HKADC_DEBUG("error reading adc channel = %d, rc = %d\n",
...
static int oem_hkadc_init_substrate_therm(void)
{
	...
	rc = oem_chg_vadc_read(LR_MUX5_AMUX_THM2, &result);
	if (rc) {
		CHG_HKADC_DEBUG("error reading adc channel = %d, rc = %d\n",
...
static int oem_hkadc_init_usb_therm(void)
{
	...
	rc = oem_chg_vadc_read(LR_MUX8_AMUX_THM4, &result);
	if (rc) {
		CHG_HKADC_DEBUG("error reading adc channel = %d, rc = %d\n",
				LR_MUX8_AMUX_THM4, rc);
		return rc;
	}
	work = (int)result.physical;
...
static void oem_hkadc_monitor(struct work_struct *work)
{
	...
	if(uevent_notify_freq >= HKADC_UEVENT_NOTIFY_COUNT) {
		...
		if(!hkadc_oem_batt_psy)
			hkadc_oem_batt_psy = power_supply_get_by_name("oem_battery");
		if(hkadc_oem_batt_psy)
			power_supply_changed(hkadc_oem_batt_psy);
	}
	else {
		if(!hkadc_batt_psy)
			hkadc_batt_psy = power_supply_get_by_name("battery");
