...
#define OEM_CHG_HVDCP_MAX_VOL_5V   5000000
#define OEM_CHG_HVDCP_MAX_VOL_9V   9000000
...
static struct power_supply *oem_charge_wlc_psy = NULL;
...
static int oem_chg_get_wlc_property(enum power_supply_property psp)
{
	union power_supply_propval ret = {0,};

	if (oem_charge_wlc_psy) {
		oem_charge_wlc_psy->get_property(oem_charge_wlc_psy, psp, &ret);
	}

	CHG_CHG_DEBUG("Get WLC Property psp = %d val = %d\n", psp, ret.intval);
...
static int oem_charger_determine_connect_state(void)
{
	...
	if (oem_chg_get_oem_stand_detect()){
		CHG_CHG_DEBUG("charger connect STAND %d \n", oem_chg_get_oem_stand_detect());
...
#ifdef CONFIG_OEM_CHARGER_WIRELESS
	if (oem_chg_get_wlc_property(POWER_SUPPLY_PROP_ONLINE)){
		CHG_CHG_DEBUG("charger connect WLC\n");
		return POWER_SUPPLY_OEM_CONNECT_CHARGE_WLC;
	}
#endif

	if (oem_charger_get_usb_property(POWER_SUPPLY_PROP_PRESENT)){
		CHG_CHG_DEBUG("charger connect USB\n");
...
int oem_charger_get_connect_state(void)
{
	...
	if (-1 < state) {
		...
		if( (OEM_CHARGE_VBATT_STATE_OFF == oem_charge_vbatt_ov_state) && (POWER_SUPPLY_OEM_CONNECT_NONE != state) ) {
			oem_charge_vbatt_ov_state = OEM_CHARGE_VBATT_STATE_ON;
			CHG_CHG_DEBUG("Set OEM_CHARGE_VBATT_STATE_ON \n");
		}
		if( (OEM_CHARGE_VBATT_STATE_ON == oem_charge_vbatt_ov_state) && (POWER_SUPPLY_OEM_CONNECT_NONE == state) ) {
			oem_charge_vbatt_ov_state = OEM_CHARGE_VBATT_STATE_OFF;
			CHG_CHG_DEBUG("Set OEM_CHARGE_VBATT_STATE_OFF \n");
		}
	} else {
		return state;
	}
...
#ifdef CONFIG_OEM_CHARGER_WIRELESS
	oem_charge_wlc_psy = power_supply_get_by_name("wlc");
#endif
...
void chg_vbatt_ov_worker(struct work_struct *work)
{
	...
	case OEM_CHARGE_VBATT_STATE_OFF:
		...
		if (oem_charge_vbatt_ov_state_now == OEM_CHARGE_VBATT_STATE_OVOFF){
			CHG_CHG_DEBUG("Illegal path charger removed\n");
	...
	case OEM_CHARGE_VBATT_STATE_OVOFF:
		...
		if (POWER_SUPPLY_OEM_CONNECT_NONE == oem_charger_determine_connect_state()){
			...
			CHG_CHG_DEBUG("Being OEM_CHARGE_VBATT_STATE_OVOFF\n");
		}
...
void oem_chg_uim_status_check(void)
{
	...
	if(*uim_status_smem_ptr != 0) {
		oem_chg_set_uim_status_fix_parm(the_chip);
		initialized = 1;
	}
}
...
void oem_chg_hvdcp_max_voltage_ctl(int dc_present)
{
	...
	if(dc_present) {
		ret.intval = OEM_CHG_HVDCP_MAX_VOL_5V;
	}
	else {
		ret.intval = OEM_CHG_HVDCP_MAX_VOL_9V;
	}
	CHG_CHG_DEBUG("Set USB VOLTAGE_MAX to %d \n", ret.intval);
	
	if(oem_charge_usb_psy) {
		oem_charge_usb_psy->set_property(oem_charge_usb_psy,
