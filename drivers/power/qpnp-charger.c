...
#endif
#ifdef CONFIG_OEM_CHARGER_WIRELESS
...
#define USB_OVP_THR				0x10
#define USB_OVP_THR_MASK		0x30
...
static int oem_after_warmcool_recharge_flag;
static void
...
qpnp_arb_stop_work(struct work_struct *work)
{
	...
	if(oem_wlc_wpc_en_ctrl_get_pri()) {
		oem_wlc_wpc_en_ctrl_set_pri(0);
...
#define OEM_STOP_THRESHOLD_PERCENTAGE	95
...
qpnp_chg_usb_chg_gone_irq_handler(int irq, void *_chip)
{
	...
	if ((qpnp_chg_is_usb_chg_plugged_in(chip)
		...
		if(OEM_STOP_THRESHOLD_PERCENTAGE <= oem_get_real_last_bms_soc_backup()) {
			oem_wlc_set_wpc_en_n(1);
			oem_wlc_wpc_en_ctrl_set_pri(1);
		}
...
qpnp_chg_vddmax_get(struct qpnp_chg_chip *chip)
{
	...
	if (rc) {
		pr_err("Failed to read vddmax: %d\n", rc);
		return rc;
	}
...
qpnp_chg_usb_usbin_valid_irq_handler(int irq, void *_chip)
{
	...
	if (chip->usb_present ^ usb_present) {
		...
		power_supply_changed(&chip->oem_batt_psy);
...
static enum power_supply_property oem_batt_power_props[] = {
...
#ifdef CONFIG_OEM_CHARGER_WIRELESS
static enum power_supply_property pm_power_props_wlc[] = {
	POWER_SUPPLY_PROP_ONLINE,
};
#endif
...
#ifdef FEATURE_QUALCOMM_ORIGINAL
static int
...
get_prop_batt_status(struct qpnp_chg_chip *chip)
{
	...
	if (oem_wlc_stage_check(STAGE_GET, STAGE_NOTHING)
...
get_prop_capacity(struct qpnp_chg_chip *chip)
{
	...
	if (chip->bms_psy) {
		...
		if ((oem_after_warmcool_recharge_flag == OEM_AFTER_WARMCOOL_CHARGE_DONE_ON_NOT_NORMAL) &&
			(chip->bat_is_cool == false) &&
			(chip->bat_is_warm == false)) {
			oem_last_bms_soc = oem_get_real_last_bms_soc_backup();
			pr_debug("oem_last_bms_soc %d soc\n", oem_last_bms_soc);
		}else {
			oem_last_bms_soc = soc;
			pr_debug("soc %d%% soc\n", soc);
		}
		...
		if (battery_status != POWER_SUPPLY_STATUS_CHARGING
				...
				&& oem_last_bms_soc <= chip->soc_resume_limit) {
...
static int
oem_batt_power_get_property(struct power_supply *psy,
				       enum power_supply_property psp,
				       union power_supply_propval *val)
{
	int result = 0;
		...
		if (!(buck_sts & VDD_LOOP_IRQ)) {
			...
			if (count == CONSECUTIVE_COUNT) {
				if (!chip->bat_is_cool && !chip->bat_is_warm) {
					...
					chip->chg_done = true;		//Evenwhen it is cool or warm, the charge status should be DONE.
				}
...
{
	struct qpnp_chg_chip *chip = _chip;
	pr_debug("buck_ctrl disable = %d, therm_lvl_sel = %d, thermal_levels = %d\n",
			disable, chip->therm_lvl_sel, chip->thermal_levels);

	if ((chip->therm_lvl_sel == (chip->thermal_levels - 1)) && !disable) {
		return 0;
	}
...
unsigned int oem_vddmax_get(void *_chip)
{
	...
	if(false == oem_is_uim_dete()) {
		...
		vddmax = chip->cool_bat_mv;
	}
	else if(chip->bat_is_warm) {
		vddmax = chip->warm_bat_mv;
	}
	else {
		vddmax = chip->max_voltage_mv;
	}
	return vddmax;
}
...
int oem_chg_get_prop_batt_temp(void *_chip)
{
	...
	return get_prop_batt_temp(chip);
}
...
#ifdef CONFIG_OEM_CHARGER_WIRELESS
		dc_online &= oem_chg_get_oem_stand_detect();
#endif
		chip->usb_psy->get_property(chip->usb_psy,
		  POWER_SUPPLY_PROP_ONLINE, &ret);
...
#else
		val->intval = dc_online | usb_ac_online;
#endif
...
#ifdef CONFIG_OEM_CHARGER_WIRELESS
static
int oem_power_get_property_wlc(struct power_supply *psy,
							enum power_supply_property psp,
							union power_supply_propval *val)
{
	int result = 0;
...
qpnp_charger_probe(struct spmi_device *spmi)
{
	...
	if (&chip->oem_batt_psy) {
		chip->oem_batt_psy.name = "oem_battery";
		chip->oem_batt_psy.type = POWER_SUPPLY_TYPE_OEM_BATTERY;
		chip->oem_batt_psy.properties = oem_batt_power_props;
		chip->oem_batt_psy.num_properties = ARRAY_SIZE(oem_batt_power_props);
		chip->oem_batt_psy.get_property = oem_batt_power_get_property;

		rc = power_supply_register(chip->dev, &chip->oem_batt_psy);
		if (rc < 0) {
			pr_err("oem_batt failed to register rc = %d\n", rc);
			goto unregister_dc_psy;
		}
	}
	...
	if (&chip->wlc_psy) {
		...
		chip->wlc_psy.properties = pm_power_props_wlc;
		chip->wlc_psy.num_properties = ARRAY_SIZE(pm_power_props_wlc);
		chip->wlc_psy.get_property = oem_power_get_property_wlc;
...
#endif
unregister_oem_batt_psy:
	if (&chip->oem_batt_psy)
		power_supply_unregister(&chip->oem_batt_psy);
unregister_dc_psy:
	if (chip->dc_chgpth_base)
