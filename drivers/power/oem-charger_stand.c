...
#define IBATMAX_MIN                  200
#define IBATMAX_500                  500
#define IBATMAX_1000                 1000
#define GPIO_BAT_OVP_RST_N           9
#define GPIO_CHG_TBATT2_ON           77
#define GPIO_CHG_TBATT2              75
...
#define THRESHOLD_IBAT_STAND_CHARGE_STOP_HI (-3400000)
...
static void oem_stand_check(void)
{
	...
	if (((DETECT_LO_FIX == oem_tbatt2_detect) && (1 != oem_charge_stand_flag)) &&
		...
		ret = oem_chg_vadc_read(DCIN, &result);
		if (ret) {
...
int oem_chg_stand_on(void)
{
	...
	rc = oem_chg_ibatmax_set(the_chip, IBATMAX_MIN);
	if (rc) {
		CHG_STAND_ERR("Failed to set ibatmax rc=%d IBATMAX_MIN\n", rc);
	}
...
static bool oem_chg_dcin_check(void)
{
	...
	rc = oem_chg_vadc_read(DCIN, &result);
	if (rc) {
		CHG_STAND_ERR("Failed to reading dcin, rc = %d\n", rc);
	}
