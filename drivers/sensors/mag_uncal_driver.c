    ...
    size_t count )
{
    ...
    if((enable == 0) && (tmp_status & (BATCH_ON << SENSOR_MAG_UNCAL))){
        batch_info.flags = 0;
        batch_info.period_ns = 0;
        batch_info.timeout =0;

        sensor_set_batch( SENSOR_MAG_UNCAL, batch_info);
    }
...
static void mag_uncal_poll_work_func(struct work_struct *work)
{
    ...
    if(sns_get_reset_status() == false){
        ...
        SENSOR_N_LOG("read_data: X[%d] Y[%d] Z[%d] XCAL[%d] YCAL[%d] ZCAL[%d]",
                     (int)read_data.mag_uncal_data.x,
                     (int)read_data.mag_uncal_data.y,
                     (int)read_data.mag_uncal_data.z,
                     (int)read_data.mag_uncal_data.cal_x,
                     (int)read_data.mag_uncal_data.cal_y,
                     (int)read_data.mag_uncal_data.cal_z );

        if( 0 == ret) {
            input_report_abs((mag_uncal_input_info.dev), ABS_X, read_data.mag_uncal_data.x);
            input_report_abs((mag_uncal_input_info.dev), ABS_Y, read_data.mag_uncal_data.y);
            input_report_abs((mag_uncal_input_info.dev), ABS_Z, read_data.mag_uncal_data.z);
            input_report_abs((mag_uncal_input_info.dev), ABS_RX, read_data.mag_uncal_data.cal_x);
            input_report_abs((mag_uncal_input_info.dev), ABS_RY, read_data.mag_uncal_data.cal_y);
            input_report_abs((mag_uncal_input_info.dev), ABS_RZ, read_data.mag_uncal_data.cal_z);

            input_sync((mag_uncal_input_info.dev));
