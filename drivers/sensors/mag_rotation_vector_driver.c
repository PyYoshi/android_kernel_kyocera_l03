...
static int mag_rot_vctr_rudder_cnt =0;
    ...
    size_t count )
{
    ...
    if((enable == 0) && (tmp_status & (BATCH_ON << SENSOR_MAG_ROT_VCTR))){
        batch_info.flags = 0;
        batch_info.period_ns = 0;
        batch_info.timeout =0;

        sensor_set_batch( SENSOR_MAG_ROT_VCTR, batch_info);
    }
...
static void mag_rot_vctr_poll_work_func(struct work_struct *work)
{
    ...
    if(sns_get_reset_status() == false){
        ret = sensor_type_get_data( SENSOR_MAG_ROT_VCTR, &read_data);
        SENSOR_N_LOG("sensor_type_get_data() ret[%d]",ret);
        SENSOR_N_LOG("read_data: X[%d] Y[%d] Z[%d] S[%d] AC[%d]",
                     (int)read_data.mag_rot_vctr_data.x,
                     (int)read_data.mag_rot_vctr_data.y,
                     (int)read_data.mag_rot_vctr_data.z,
                     (int)read_data.mag_rot_vctr_data.s,
                     (unsigned char)read_data.mag_rot_vctr_data.accuracy );

        if( 0 == ret) {
            input_report_abs( mag_rot_vctr_input_info.dev,
                              ABS_X,
                              read_data.mag_rot_vctr_data.x );
            input_report_abs( mag_rot_vctr_input_info.dev,
                              ABS_Y,
                              read_data.mag_rot_vctr_data.y );
            input_report_abs( mag_rot_vctr_input_info.dev,
                              ABS_Z,
                              read_data.mag_rot_vctr_data.z );
            input_report_abs( mag_rot_vctr_input_info.dev,
                              ABS_RX,
                              read_data.mag_rot_vctr_data.s );
            input_report_abs( mag_rot_vctr_input_info.dev,
                              ABS_STATUS,
                              read_data.mag_rot_vctr_data.accuracy );
            if( (mag_rot_vctr_last_read_data.x
                     == read_data.mag_rot_vctr_data.x)
                 && (mag_rot_vctr_last_read_data.y
                     == read_data.mag_rot_vctr_data.y)
                 && (mag_rot_vctr_last_read_data.z
                ...
                SENSOR_A_LOG( "report grv - ABS_RUDDER: cnt[%d]",
                              (int)(mag_rot_vctr_rudder_cnt) );
                input_report_abs( mag_rot_vctr_input_info.dev,
                                  ABS_RUDDER,
                                 (mag_rot_vctr_rudder_cnt++) );
            }
            input_sync(mag_rot_vctr_input_info.dev);

            memcpy( &mag_rot_vctr_last_read_data,
                    &(read_data.mag_rot_vctr_data),
                    sizeof(read_data.mag_rot_vctr_data) );
        }
    }
