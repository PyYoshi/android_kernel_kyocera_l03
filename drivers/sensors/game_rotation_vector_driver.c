...
static int game_rot_vctr_rudder_cnt =0;
    ...
    size_t count )
{
    ...
    if((enable == 0) && (tmp_status & (BATCH_ON << SENSOR_GAME_ROT_VCTR))){
        batch_info.flags = 0;
        batch_info.period_ns = 0;
        batch_info.timeout =0;

        sensor_set_batch( SENSOR_GAME_ROT_VCTR, batch_info);
    }
...
static void game_rot_vctr_poll_work_func(struct work_struct *work)
{
    ...
    if(sns_get_reset_status() == false){
        ret = sensor_type_get_data( SENSOR_GAME_ROT_VCTR, &read_data);
        SENSOR_N_LOG("sensor_type_get_data() ret[%d]",ret);
        SENSOR_N_LOG("read_data: X[%d] Y[%d] Z[%d] S[%d]",
                     (int)read_data.game_rot_vctr_data.x,
                     (int)read_data.game_rot_vctr_data.y,
                     (int)read_data.game_rot_vctr_data.z,
                     (int)read_data.game_rot_vctr_data.s );

        if( 0 == ret) {
            SENSOR_A_LOG("report game rot vctr [x,y,z,s]:[%d,%d,%d,%d]",
                         (int)read_data.game_rot_vctr_data.x,
                         (int)read_data.game_rot_vctr_data.y,
                         (int)read_data.game_rot_vctr_data.z,
                         (int)read_data.game_rot_vctr_data.s );
            input_report_abs( game_rot_vctr_input_info.dev,
                              ABS_X,
                              read_data.game_rot_vctr_data.x );
            input_report_abs( game_rot_vctr_input_info.dev,
                              ABS_Y,
                              read_data.game_rot_vctr_data.y );
            input_report_abs( game_rot_vctr_input_info.dev,
                              ABS_Z,
                              read_data.game_rot_vctr_data.z );
            input_report_abs( game_rot_vctr_input_info.dev,
                              ABS_STATUS,
                              read_data.game_rot_vctr_data.s );
            if( (game_rot_vctr_last_read_data.x == read_data.game_rot_vctr_data.x)
                && (game_rot_vctr_last_read_data.y == read_data.game_rot_vctr_data.y)
                && (game_rot_vctr_last_read_data.z == read_data.game_rot_vctr_data.z) ) {
                SENSOR_A_LOG("report grv - ABS_RUDDER: cnt[%d]",
                             (int)(game_rot_vctr_rudder_cnt) );
                input_report_abs( game_rot_vctr_input_info.dev,
                                  ABS_RUDDER,
                                  (game_rot_vctr_rudder_cnt++) );
            }
            input_sync(game_rot_vctr_input_info.dev);

            memcpy(&game_rot_vctr_last_read_data,&(read_data.game_rot_vctr_data),
                   sizeof(read_data.game_rot_vctr_data));
        }
    }
