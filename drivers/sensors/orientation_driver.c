    ...
    size_t count )
{
    ...
    if((enable == 0) && (tmp_status & (BATCH_ON << SENSOR_ORTN))){
        batch_info.flags = 0;
        batch_info.period_ns = 0;
        batch_info.timeout =0;

        sensor_set_batch( SENSOR_ORTN, batch_info);
    }
...
static void ortn_poll_work_func(struct work_struct *work)
{
    ...
    if(sns_get_reset_status() == false){
        ...
        SENSOR_N_LOG("read_data: pitch[%d] roll[%d] yaw[%d] AC[%d]",
                     (int)read_data.ortn_data.pitch,
                     (int)read_data.ortn_data.roll,
                     (int)read_data.ortn_data.yaw,
                     (unsigned char)read_data.ortn_data.accuracy);

        if( 0 == ret) {
            sensor_report_ac_abs( ortn_input_info.dev,
                               read_data.ortn_data.yaw,
                               read_data.ortn_data.pitch,
                               read_data.ortn_data.roll,
                               ortn_last_read_data.yaw,
                               ortn_last_read_data.pitch,
                               ortn_last_read_data.roll,
                               read_data.ortn_data.accuracy,
                               &ortn_rudder_cnt );
