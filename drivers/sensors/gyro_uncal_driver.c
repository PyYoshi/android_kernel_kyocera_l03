    ...
    size_t count )
{
    ...
    if((enable == 0) && (tmp_status & (BATCH_ON << SENSOR_GYRO_UNCAL))){
        batch_info.flags = 0;
        batch_info.period_ns = 0;
        batch_info.timeout =0;

        sensor_set_batch( SENSOR_GYRO_UNCAL, batch_info);
    }
...
static void gyro_uncal_poll_work_func(struct work_struct *work)
{
    ...
    if(sns_get_reset_status() == false){
        ...
        SENSOR_N_LOG("read_data: X[%d] Y[%d] Z[%d] XCAL[%d] YCAL[%d] ZCAL[%d]",
                     (int)read_data.gyro_uncal_data.x,
                     (int)read_data.gyro_uncal_data.y,
                     (int)read_data.gyro_uncal_data.z,
                     (int)read_data.gyro_uncal_data.cal_x,
                     (int)read_data.gyro_uncal_data.cal_y,
                     (int)read_data.gyro_uncal_data.cal_z );

        if( 0 == ret) {
            SENSOR_A_LOG("report gyro uncal [x,y,z,calx,caly,calz]:[%d,%d,%d,%d,%d,%d]",
                     (int)read_data.gyro_uncal_data.x,
                     (int)read_data.gyro_uncal_data.y,
                     (int)read_data.gyro_uncal_data.z,
                     (int)read_data.gyro_uncal_data.cal_x,
                     (int)read_data.gyro_uncal_data.cal_y,
                     (int)read_data.gyro_uncal_data.cal_z );
            input_report_abs((gyro_uncal_input_info.dev), ABS_X, read_data.gyro_uncal_data.x);
            input_report_abs((gyro_uncal_input_info.dev), ABS_Y, read_data.gyro_uncal_data.y);
            input_report_abs((gyro_uncal_input_info.dev), ABS_Z, read_data.gyro_uncal_data.z);
            input_report_abs((gyro_uncal_input_info.dev), ABS_RX, read_data.gyro_uncal_data.cal_x);
            input_report_abs((gyro_uncal_input_info.dev), ABS_RY, read_data.gyro_uncal_data.cal_y);
            input_report_abs((gyro_uncal_input_info.dev), ABS_RZ, read_data.gyro_uncal_data.cal_z);

            input_sync((gyro_uncal_input_info.dev));
