    ...
    char *buf )
{
    ...
    ret = sprintf( buf, "%d %d %d\n",
                   acc_last_read_data.nX,
                   acc_last_read_data.nY,
                   acc_last_read_data.nZ );
    ...
    size_t count )
{
    ...
    if((enable == 0) && (tmp_status & (BATCH_ON << SENSOR_ACC))){
        batch_info.flags = 0;
        batch_info.period_ns = 0;
        batch_info.timeout =0;

        sensor_set_batch( SENSOR_ACC, batch_info);
    }
...
static ssize_t acc_auto_cal_offset_store(struct device *dev,
    ...
    size_t count)
{
    int32_t auto_cal_offset;

    SENSOR_N_LOG("start");
    sscanf(buf,"%d",&auto_cal_offset);
    SENSOR_N_LOG("acc_auto_cal_offset : %d",auto_cal_offset);
...
static void acc_poll_work_func(struct work_struct *work)
{
    ...
    if(sns_get_reset_status() == false){
        ret = sensor_type_get_data( SENSOR_ACC, &read_data);
        SENSOR_N_LOG("sns_acc_read_data() ret[%d]",ret);
        SENSOR_N_LOG("read_data: nX[%d] nY[%d] nZ[%d]",
                     (int)read_data.acc_data.nX,
                     (int)read_data.acc_data.nY,
                     (int)read_data.acc_data.nZ );
        SENSOR_N_LOG("read_data: outX[%d] outY[%d] outZ[%d]",
                     (int)read_data.acc_data.outX,
                     (int)read_data.acc_data.outY,
                     (int)read_data.acc_data.outZ);

        if( 0 == ret) {

            SENSOR_A_LOG("report ACC [x,y,z]:[%d,%d,%d]",
                         (int)read_data.acc_data.outX,
                         (int)read_data.acc_data.outY,
                         (int)read_data.acc_data.outZ );
            input_report_abs((acc_input_info.dev), ABS_X, read_data.acc_data.outX);
            input_report_abs((acc_input_info.dev), ABS_Y, read_data.acc_data.outY);
            input_report_abs((acc_input_info.dev), ABS_Z, read_data.acc_data.outZ);

            if( (acc_last_read_data.outX == read_data.acc_data.outX)
                && (acc_last_read_data.outY == read_data.acc_data.outY)
                && (acc_last_read_data.outZ == read_data.acc_data.outZ) ) {
                SENSOR_A_LOG("report ACC - ABS_RUDDER: cnt[%d]",
                             (int)acc_rudder_cnt );
                input_report_abs( acc_input_info.dev,
                                  ABS_RUDDER,
                                  acc_rudder_cnt++ );
