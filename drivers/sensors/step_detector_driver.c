    ...
    size_t count )
{
    ...
    SENSOR_N_LOG("start step_dtc_delay_time:%d delay_time:%d",step_dtc_delay_time,delay_time);

    if( delay_time > 30){
        delay_time = 30;
    }
    
    step_dtc_delay_time = delay_time;
    ...
    size_t count )
{
    ...
    batch_info.flags = 0;
    batch_info.period_ns = step_dtc_delay_time;

    if( true == enable ) {
        batch_info.timeout = step_dtc_delay_time;
    } else {
