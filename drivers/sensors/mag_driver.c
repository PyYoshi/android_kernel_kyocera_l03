    ...
    char *buf )
{
    ...
    if(ret == SNS_RC_OK) {
        mag_offset.hard_offset[0] = offsets[0];
        mag_offset.hard_offset[1] = offsets[1];
        mag_offset.hard_offset[2] = offsets[2];
        mag_offset.calib_offset.v[0] = offsets[3];
        mag_offset.calib_offset.v[1] = offsets[4];
        mag_offset.calib_offset.v[2] = offsets[5];
        mag_last_status = accuracy;
    }

    ret = sprintf( buf, "%d %d %d %d %d %d %d\n",
                   mag_offset.hard_offset[0],
                   mag_offset.hard_offset[1],
                   mag_offset.hard_offset[2],
                   mag_offset.calib_offset.v[0],
                   mag_offset.calib_offset.v[1],
                   mag_offset.calib_offset.v[2],
                   mag_last_status );

    SENSOR_N_LOG("read_data: HO1[%d] HO2[%d] HO3[%d] CO1[%d] CO2[%d] CO3[%d] acc[%d]",
                 mag_offset.hard_offset[0],
                 mag_offset.hard_offset[1],
                 mag_offset.hard_offset[2],
                 mag_offset.calib_offset.v[0],
   ...
   size_t count )
{
    ...
    if (0 <= accuracy && accuracy <= 3) {
        for (i = 0; i < 3; i++){
            mag_offset.hard_offset[i] = (int8_t)hard_offset[i];
        }
        mag_last_status = accuracy;
    }

    offsets[0] = mag_offset.hard_offset[0];
    offsets[1] = mag_offset.hard_offset[1];
    offsets[2] = mag_offset.hard_offset[2];
    offsets[3] = mag_offset.calib_offset.v[0];
    offsets[4] = mag_offset.calib_offset.v[1];
    offsets[5] = mag_offset.calib_offset.v[2];
    ...
    size_t count )
{
    ...
    if((enable == 0) && (tmp_status & (BATCH_ON << SENSOR_MAG))){
        batch_info.flags = 0;
        batch_info.period_ns = 0;
        batch_info.timeout =0;

        sensor_set_batch( SENSOR_MAG, batch_info);
    }
...
static void mag_poll_work_func(struct work_struct *work)
{
    ...
    if(sns_get_reset_status() == false){
        ...
        SENSOR_N_LOG("read_data: X[%d] Y[%d] Z[%d] AC[%d]",
                     (int)read_data.mag_data.x,
                     (int)read_data.mag_data.y,
                     (int)read_data.mag_data.z,
                     (unsigned char)read_data.mag_data.accuracy);

        if( 0 == ret) {
            sensor_report_ac_abs( mag_input_info.dev,
                               read_data.mag_data.x,
                               read_data.mag_data.y,
                               read_data.mag_data.z,
                               mag_last_read_data.x,
                               mag_last_read_data.y,
                               mag_last_read_data.z,
                               read_data.mag_data.accuracy,
                               &mag_rudder_cnt );
