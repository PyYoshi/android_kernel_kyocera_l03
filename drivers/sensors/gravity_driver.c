    ...
    size_t count )
{
    ...
    if((enable == 0) && (tmp_status & (BATCH_ON << SENSOR_GRV))){
        batch_info.flags = 0;
        batch_info.period_ns = 0;
        batch_info.timeout =0;

        sensor_set_batch( SENSOR_GRV, batch_info);
    }
...
static void grv_poll_work_func(struct work_struct *work)
{
    ...
    if(sns_get_reset_status() == false){
        ret = sensor_type_get_data( SENSOR_GRV, &read_data);
        SENSOR_N_LOG("sensor_type_get_data() ret[%d]",ret);
        SENSOR_N_LOG("read_data: X[%d] Y[%d] Z[%d]",
                     (int)read_data.grv_data.x,
                     (int)read_data.grv_data.y,
                     (int)read_data.grv_data.z );

        if( 0 == ret) {
            sensor_report_abs( grv_input_info.dev,
                               read_data.grv_data.x,
                               read_data.grv_data.y,
                               read_data.grv_data.z,
                               grv_last_read_data.x,
                               grv_last_read_data.y,
                               grv_last_read_data.z,
                               &grv_rudder_cnt );
            memcpy(&grv_last_read_data,&(read_data.grv_data),sizeof(read_data.grv_data));
        }
    }
