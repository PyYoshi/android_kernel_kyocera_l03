...
static void gyro_poll_work_func(struct work_struct *work)
{
    ...
    if(sns_get_reset_status() == false){
        ...
        SENSOR_N_LOG("read_data: X[%d] Y[%d] Z[%d]",
                     (int)read_data.gyro_data.x,
                     (int)read_data.gyro_data.y,
                     (int)read_data.gyro_data.z );

        if( 0 == ret) {
            SENSOR_A_LOG("report gyro [x,y,z]:[%d,%d,%d]",
                         (int)read_data.gyro_data.x,
                         (int)read_data.gyro_data.y,
                         (int)read_data.gyro_data.z);
            input_report_abs((gyro_input_info.dev), ABS_X, read_data.gyro_data.x);
            input_report_abs((gyro_input_info.dev), ABS_Y, read_data.gyro_data.y);
            input_report_abs((gyro_input_info.dev), ABS_Z, read_data.gyro_data.z);

            input_sync((gyro_input_info.dev));
        }
    }
