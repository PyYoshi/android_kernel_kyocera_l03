...
static int acc_lnr_rudder_cnt =0;
    ...
    size_t count )
{
    ...
    if((enable == 0) && (tmp_status & (BATCH_ON << SENSOR_ACC_LNR))){
        batch_info.flags = 0;
        batch_info.period_ns = 0;
        batch_info.timeout =0;

        sensor_set_batch( SENSOR_ACC_LNR, batch_info);
    }
...
static void acc_lnr_poll_work_func(struct work_struct *work)
{
    ...
    if(sns_get_reset_status() == false){
        ret = sensor_type_get_data( SENSOR_ACC_LNR, &read_data);
        SENSOR_N_LOG("sensor_type_get_data() ret[%d]",ret);
        SENSOR_N_LOG("read_data: X[%d] Y[%d] Z[%d]",
                     (int)read_data.acc_lnr_data.x,
                     (int)read_data.acc_lnr_data.y,
                     (int)read_data.acc_lnr_data.z );

        if( 0 == ret) {
            sensor_report_abs( acc_lnr_input_info.dev,
                               read_data.acc_lnr_data.x,
                               read_data.acc_lnr_data.y,
                               read_data.acc_lnr_data.z,
                               acc_lnr_last_read_data.x,
                               acc_lnr_last_read_data.y,
                               acc_lnr_last_read_data.z,
                               &acc_lnr_rudder_cnt );

            memcpy(&acc_lnr_last_read_data,&(read_data.acc_lnr_data),sizeof(read_data.acc_lnr_data));
        }
    }
