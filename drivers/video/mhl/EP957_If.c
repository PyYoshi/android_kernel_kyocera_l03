...
#define TX_PHY_CTL_0_VAL		0x20
#define TX_PHY_CTL_1_VAL		0x0F
#define RX_PHY_CTL_0_VAL		0x0A
#define GEN_CTL_3_VAL			0x93
#ifdef CONFIG_EP957B_MHL_A
#define CBUS_BR_ADJ_VAL			42
#elif defined(CONFIG_EP957B_MHL_B)
#define CBUS_BR_ADJ_VAL			42
#endif
...
DECLARE_COMPLETION(mhl_hpd_done);

static bool is_factory_mode = false;

static bool notified_disconnect = false;
...
void (*mhl_notify_usb_online)(void *ctx, int online) = NULL;
void *mhl_notify_ctx = NULL;
...
void MHL_set_Charge_Mode(BOOL Charge_Mode)
{
	if(!mhl_notify_usb_online)
		EP_DEV_DBG_ERROR("%s:mhl_notify_usb_online is NULL\n",__func__);
	if(!mhl_notify_ctx)
		EP_DEV_DBG_ERROR("%s:mhl_notify_ctx is NULL\n",__func__);

	if(mhl_notify_usb_online && mhl_notify_ctx){
		if(Charge_Mode) {
			EP_DEV_DBG("###MHL_set_Charge_Mode on \n");
			mhl_charge_mode = 1;
			mhl_notify_usb_online(mhl_notify_ctx, 1);
		} else {
			EP_DEV_DBG("###MHL_set_Charge_Mode off \n");
			mhl_charge_mode = 0;
			mhl_notify_usb_online(mhl_notify_ctx, 0);
		}
	}
}
...
#endif
	Temp_Data[0] = CBUS_BR_ADJ_VAL; // default 47, set to 42 for E version
...
void HDMI_Rx_HotPlug_Set(void)
{
	...
	if (!is_factory_mode) {
		INIT_COMPLETION(mhl_hpd_done);
		timeout = wait_for_completion_interruptible_timeout(&mhl_hpd_done, HZ * 5);
		if (timeout < 1)
			EP_DEV_DBG_ERROR("%s: hdmi power up not completed\n", __func__);
	}
}
...
void HDMI_Rx_HotPlug_Clear(void)
{
	...
	if (!is_factory_mode) {
		...
		timeout = wait_for_completion_interruptible_timeout(&mhl_hpd_done, HZ * 5);
		if (timeout < 1)
			EP_DEV_DBG_ERROR("%s: hdmi power down not completed\n", __func__);
	}
}
...
void callback_work_Link_Task(struct work_struct *p) // modify 130218
{
	...
	{
		// Just after disconnected
		hrtimer_start(&pTimer_Link_Task, ktime_set(0, 2000*1000000), HRTIMER_MODE_REL); // 2 sec
	}
	else if(Link_State == CBUS_LINK_STATE__USB_Mode)
	{
		// USB Mode
...
static int EP957_MHL_module_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	ret = gpio_request(MHL_INT_GPIO, "MHL_INT");
	if (ret) {
		EP_DEV_DBG_ERROR("unable to request gpio MHL_INT err:: %d\n", ret);	
		return ret;
	}
...
void EP957_MHL_HDMI_power_done(void *data)
{
	EP_DEV_DBG("HDMI Power Control done\n");
	if (!is_factory_mode) {
		complete(&mhl_hpd_done);
	}
}

static int __init setup_factory_mode(char *buf)
{
...
early_param("androidboot.mode", setup_factory_mode);
