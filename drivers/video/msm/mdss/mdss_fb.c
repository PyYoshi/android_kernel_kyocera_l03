	...
	struct device_attribute *attr, const char *buf, size_t count)
{
	...
	DISP_LOCAL_LOG_EMERG("display_mipi_err_valid: %d\n"
													, disp_cfg.rw_display_mipi_err_valid);
	...
	DISP_LOCAL_LOG_EMERG("DISPLAY_MIPI_ERR: %02x\n"
													, disp_cfg.rw_display_mipi_err);
...
#ifdef CONFIG_DISP_EXT_PROPERTY
static DEVICE_ATTR(msm_fb_disp_cfg, S_IRUGO|S_IWUSR
															, NULL, msm_fb_disp_cfg_store);
#endif /* CONFIG_DISP_EXT_PROPERTY */
...
static void mdss_fb_shutdown(struct platform_device *pdev)
{
	...
	disp_ext_util_set_shutdown_pending(mfd->shutdown_pending);
			 ...
			 unsigned long arg)
{
	...
	case MSMFB_GET_STATUS:
	{
		...
		DISP_LOCAL_LOG_EMERG("%s:MSMFB_GET_STATUS start\n", __func__);

		status = disp_ext_util_get_disp_info();

		ret = copy_to_user(argp, status, sizeof(struct local_disp_state_type));
		if(ret) {
			pr_err("%s:MSMFB_GET_STATUS error\n", __func__);
			return ret;
		}

		DISP_LOCAL_LOG_EMERG("%s:MSMFB_GET_STATUS end\n", __func__);
#else /* CONFIG_DISP_EXT_UTIL */
        DISP_LOCAL_LOG_EMERG("%s:DISP MSMFB_GET_STATUS\n", __func__);
		ret = -1;
	...
	case MSMFB_GET_REF_SW:
	{
		...
		DISP_LOCAL_LOG_EMERG("%s:MSMFB_GET_REF_SW start\n", __func__);

		pdata = dev_get_platdata(&mfd->pdev->dev);
		lock_fb_info(mfd->fbi);
		disp_ext_refresh_ctrl_start(pdata);
		unlock_fb_info(mfd->fbi);

		refresh_sw = disp_ext_get_refresh_start_sw();
		ret = copy_to_user(argp, &refresh_sw, sizeof(uint8_t));
		if(ret) {
			pr_err("%s:MSMFB_GET_REF_SW error\n", __func__);
			return ret;
		}

		DISP_LOCAL_LOG_EMERG("%s:MSMFB_GET_REF_SW end\n", __func__);
	...
	case MSMFB_REFRESH_SEQ:
	{
		...
		DISP_LOCAL_LOG_EMERG("%s:MSMFB_REFRESH_SEQ start\n", __func__);

		pdata = dev_get_platdata(&mfd->pdev->dev);
		if(NULL == pdata) {
			pr_err("%s:ERROR. pdata is NULL.\n", __func__);
			return -ENODEV;
		}
		...
		if(LOCAL_DISPLAY_ON == disp_ext_util_get_disp_state()) {
			ret = disp_ext_refresh_seq_exec(pdata);
		}
		unlock_fb_info(mfd->fbi);
		DISP_LOCAL_LOG_EMERG("%s:MSMFB_REFRESH_SEQ end\n", __func__);
