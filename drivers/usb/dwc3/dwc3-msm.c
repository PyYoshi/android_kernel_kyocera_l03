...
static int override_phy_init = 0x03DCCEE4;
...
struct work_struct dwc3_ext_notify_work;
...
static bool msm_register_usb_ext_notification_wait = false;
...
int msm_register_usb_ext_notification(struct usb_ext_notification *info)
{
	...
	if( context != NULL ) {
		usb_ext->notify(usb_ext->ctxt, 0, dwc3_ext_notify_online, context);
	}
	else {
		pr_info("%s:Run later registration.\n",__func__);
		msm_register_usb_ext_notification_wait = true;
	}
...
static void dwc3_ext_notify_register_work(struct work_struct *data)
{
	if( usb_ext != NULL ) {
		pr_err("%s:To perform the registration.\n",__func__);
		usb_ext->notify(usb_ext->ctxt, 0, dwc3_ext_notify_online, context);
	}
}
...
static int dwc3_msm_suspend(struct dwc3_msm *mdwc)
{
	...
	dev_notice(mdwc->dev, "DWC3 in low power mode\n");
...
get_prop_usbin_voltage_now(struct dwc3_msm *mdwc)
{
	...
	msleep(200); // put wait here such as 200 (msec) 
...
static int __devinit dwc3_msm_probe(struct platform_device *pdev)
{
	...
	if( msm_register_usb_ext_notification_wait == true ) {
		INIT_WORK(&dwc3_ext_notify_work, dwc3_ext_notify_register_work);
		schedule_work(&dwc3_ext_notify_work);
		msm_register_usb_ext_notification_wait = false;
	}
	init_completion(&mdwc->ext_chg_wait);
